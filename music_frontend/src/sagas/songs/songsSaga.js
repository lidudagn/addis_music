...
function* WorkGetSongsFetch() {
  ...
  const songs = yield call(() => fetch("https://addis-music-one.vercel.app/songs"));
...
function* workPostSong(action) {
  console.log(action.payload, "paylod");
   const response = yield call(() =>
     fetch("https://addis-music-one.vercel.app/songs", {
       method: "POST",
...
function* workDeleteSong(action) {
  const response = yield call(() =>
    fetch(`https://addis-music-one.vercel.app/songs/${action.payload}`, {
      method: "DELETE",
    })
  );
...
function* workUpdateSong(action) {
  const response = yield call(() =>
    fetch(`https://addis-music-one.vercel.app/songs/${action.payload.id}`, {
      method: "PUT",
      body: JSON.stringify(action.payload),
      headers: {
        "Content-Type": "application/json",
